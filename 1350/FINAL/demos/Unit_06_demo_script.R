...
YVR_Air_Temps <- read.delim("S:/Personal Folders/BCIT_W2022_MATH_1350/W2022_MATH_1350_Data/YVR_Air_Temps.txt")
...
with(YVR_Air_Temps,
     lines(Year[Year > 2000],
          Jan.Temp[Year > 2000],'b',col="black"))
...
#the distribution of p-hat comes from the binomial
#distribution:
...
#Now generate the sampling distribution of p-hat based 
#on 1 million random samples
p <- 0.6
q <- 1-p
n <- 100
...
pdf.vals.norm <- dnorm(X.vals.norm, mean=mu.phat,
                       sd=sig.phat)
lines(X.vals.norm, pdf.vals.norm, col="red")
...
data <- read_excel(
  "S:/Personal Folders/BCIT_W2022_MATH_1350/W2022_MATH_1350_Data/F2021_MATH_1350_Data.xlsx")
...
sig.X
sd(X.height) #note that sd(X) gives a different value
...
X.vals <- seq(-5.5,5.5,0.1)
lines(X.vals, dnorm(X.vals),col="black",type="l",lwd=1)
